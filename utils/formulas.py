...
def _resolve_damage(attacker, defender, move, weather='none', terrain='none', target_hazards=None, user_hazards=None, wonder_room=False, gravity=False, magic_room=False, user_party=None, field=None):
    ...
    # **THE TERA DOOR.** `battle_types` answers "what does this count as right now" -
    # Terastallised typing replaces the lot, and Mimicry is asked only when the crystal is
    # not out. STAB is NOT read off these: a Terastallised attacker's originals still earn
    # the bonus, which `tera.stab_multiplier` works out from both.
    atk_types = tera_battle_types(attacker, terrain)
    def_types = tera_battle_types(defender, terrain)
    ...
    if move.get('class') != 'status' and (move.get('power', 0) > 0 or dynamic_power):
        ...
        # Two-sided once the crystal is out: the Tera type and the original typing both
        # earn the bonus, and agreeing earns more than either. See utils/tera.py.
        stab = tera_stab_multiplier(attacker, move_type,
                                    adaptability=(atk_ability == 'adaptability'),
                                    terrain=terrain)
